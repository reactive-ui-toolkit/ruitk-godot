...
## no need to hold a reference yourself (unlike the bare `RuitkRoot`).
##
## Usage (code):
##   var r := RuitkRootNode.new().setup(MyApp.render, { "title": "Hi" })
##   add_child(r)
##
## Usage (scene): attach a script `extends RuitkRootNode` and override `build()`.

var root_component: Callable
var root_props: Dictionary = {}
var _app: RuitkRoot = null

## Set the root component + props before entering the tree. Returns self for chaining.
func setup(component: Callable, props := {}) -> RuitkRootNode:
	root_component = component
	root_props = props
	return self

## Override in a subclass to return the root vnode (alternative to `setup`).
func build() -> RuitkVNode:
...
func _ready() -> void:
	var vnode := build()
	if vnode != null:
		_app = RuitkRoot.create(self, vnode)
