...
func show_help() -> void:
	...
	for line in [
		...
		"CARDS — drag the TITLE BAR to move a card; drag the KIND CHIP to carry the module itself,",
		"        onto a folder row to re-file it or onto another card to import it.",
		"        Drag a row to reorder it. Right-click a card to open, rename or delete it.",
		"        Double-click a card to frame it; double-click a <Component> row to go to its module.",
		"        A long section scrolls; over one, the plain wheel scrolls and Ctrl+wheel still zooms.",
		"FOLDERS — drag a module onto a folder row, or onto any file row to join its folder.",
		"          A refused drop says which refusal. The pane folds from its own header.",
		"EDITING — click an attribute or a badge to edit it in place; the source pane edits the same buffer.",
		"          Enter finishes an edit; on a style entry it opens the next one. Escape cancels,",
		"          and cancelling an edit the builder seeded takes the seeding back with it.",
		"          Double-click the source listing to start editing it; Ctrl+click a name to go to it.",
		"HISTORY — Ctrl+Z and Ctrl+Shift+Z walk the ledger; the History menu jumps to any point in it.",
