...
**Update renders are now time-sliced by default.** Re-renders run in 2 ms quanta on the new four-lane `RuitkScheduler` under a 4 ms/frame budget, so one big update can't stall a frame. Commits stay atomic (never a half-applied tree), mounts stay synchronous (first frames paint whole), and updates landing mid-pass defer and replay after commit as ONE follow-up render — with a depth-25 runaway guard. Opt out anytime: `RuitkConfig.time_slicing = false` (or the Project Setting) restores the classic sync render, untouched by this release. Bench (keyed-list stress, ms/frame): N=1500 12.2 → 6.9, N=3000 26.5 → 14.7; the sync arm measures within noise of its baseline.

**Error boundaries are real now.** A failing render calls `RuitkFail.render(reason)` and the nearest `V.error_boundary` shows its fallback in the same commit — mount or update, sliced or sync. (A hard GDScript crash still can't be auto-caught.)

**Also new:** strict mode (opt-in double-render impure-render flusher, forced off in release builds), missing-dependency warnings under strict diagnostics, a trace ladder (`trace_level` none/basic/verbose + independent `diff_tracing`), and an `environment` label (`development`/`production`) for your own debug overlays.
