  ...
  <Box sx={Styles.root}>
    ...
    <List>
      <ListItem disablePadding>
        <ListItemText
          primary="From the Asset Library: open the AssetLib tab, search “Reactive UI Toolkit — Godot”, then Download → Install (keep the addons/reactive_ui_toolkit/ folder). The editor tooling ships as a separate asset — search “Reactive UI Toolkit — Godot Editor” to add it too."
          secondary="Or, manually: copy the addons/reactive_ui_toolkit/ folder into your project's res://addons/ directory."
        />
      </ListItem>
      <ListItem disablePadding>
    ...
    <Typography variant="body1" paragraph>
      ...
      <code>Hooks</code>, <code>RuitkRoot</code>, …), so the runtime is available immediately — no
    ...
    <Typography variant="body1" paragraph>
      ...
      the <code>{'-> RuitkVNode'}</code> return annotation is what marks a declaration as a
    ...
    <Typography variant="body1" paragraph>
      ...
      <code>static func render(props, children) -&gt; RuitkVNode</code> method. The generated file is
    ...
    <Typography variant="body1" paragraph>
      ...
      function-component <code>RuitkVNode</code> — the reconciler's entry point into your component
      tree. Mount it under any <code>Control</code> with <code>RuitkRoot.create</code>:
    </Typography>
    <CodeBlock language="gdscript" code={UITKX_HELLO_WORLD_BOOTSTRAP} />
    <Typography variant="body1" paragraph>
      <code>RuitkRoot.create(container, root_vnode)</code> mounts under <code>container</code> and
      renders. <strong>Hold onto the returned <code>RuitkRoot</code></strong> (for example in a
    ...
    <Typography variant="body1" paragraph>
      If you prefer mounting through a node in the scene, use <code>RuitkRootNode</code> — a{' '}
